...
function SaveBannerAllSizes($request, $dir){
    ...
    $lg_image = Image::make($request)->fit(400);
    $lg_image->response('png');

    $md_image = Image::make($request)->fit(200);
    $md_image->response('png');

    $sm_image = Image::make($request)->fit(100);
    $sm_image->response('png');

    $xs_image = Image::make($request)->fit(50);
    $xs_image->response('png');

    $image_name = $request->hashName();
